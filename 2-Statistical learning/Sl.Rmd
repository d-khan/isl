...
__Answer:__ Worse. In non-parametric methods, the accuracy of function (f) depends upon the large number of observations. However in order for flexible statistical learning to perform better, large number of predictors are required.    

(b) The number of predictors _p_ is extremely large, and the number of observations _n_ is small.  
__Answer:__ Better. Generally flexible learning methods needs large number of predictors to perform better. With respect to this question in particular, only parametric methods will be used since number of observations are small. 

(c) The relationship between the predictors and response is highly non-linear.  
__Answer:__ Better. Generally flexible learning works better when the response in non-linear. However keep in mind the U-shape characteristics for test error and bias-variance trade off which provides upper limit in terms of performance for flexible methods.    
...
__Answer:__ Classification problem, prediction, n=20, p_1=success/failure, p_2=price charged, p_3=marketing budget, p_4=competition price, p_5, ... ,p_15.  
